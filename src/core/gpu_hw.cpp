...
bool GPU_HW::Initialize(bool clear_vram, Error* error)
{
  ...
  if (!CompilePipelines())
  {
    Error::SetStringView(error, "Failed to compile pipelines");
    return false;
  }
...
void GPU_HW::UpdateSettings(const Settings& old_settings)
{
  ...
  const bool framebuffer_changed = (m_resolution_scale != resolution_scale || m_multisamples != multisamples ||
                                    (static_cast<bool>(m_vram_depth_texture) !=
                                     (g_gpu_settings.UsingPGXPDepthBuffer() || !m_supports_framebuffer_fetch)));
  const bool shaders_changed =
    (m_resolution_scale != resolution_scale || m_multisamples != multisamples ||
     m_true_color != g_gpu_settings.gpu_true_color || g_gpu_settings.gpu_debanding != old_settings.gpu_debanding ||
  ...
  if (framebuffer_changed)
  {
    ...
    UpdateDownsamplingLevels();
    RestoreDeviceContext();
    UpdateVRAM(0, 0, VRAM_WIDTH, VRAM_HEIGHT, g_vram, {});
    UpdateDepthBufferFromMaskBit();
  }
...
u32 GPU_HW::CalculateResolutionScale() const
{
  ...
  else
  {
    ...
    const s32 height = (m_display_height != 0) ? static_cast<s32>(m_display_height) :
                                                 ((System::GetRegion() == ConsoleRegion::PAL) ?
    ...
    if (g_gpu_settings.gpu_widescreen_hack)
    {
...
std::tuple<u32, u32> GPU_HW::GetEffectiveDisplayResolution(bool scaled /* = true */) const
{
  const u32 scale = scaled ? m_resolution_scale : 1u;
  return std::make_tuple(m_display_vram_width * scale, m_display_vram_height * scale);
}

std::tuple<u32, u32> GPU_HW::GetFullDisplayResolution(bool scaled /* = true */) const
{
  const u32 scale = scaled ? m_resolution_scale : 1u;
  return std::make_tuple(m_display_width * scale, m_display_height * scale);
}
...
void GPU_HW::PrintSettingsToLog()
{
  ...
  INFO_LOG("Dithering: {}{}", m_true_color ? "Disabled" : "Enabled",
           (!m_true_color && g_gpu_settings.gpu_scaled_dithering) ?
             " (Scaled)" :
             ((m_true_color && g_gpu_settings.gpu_debanding) ? " (Debanding)" : ""));
...
void GPU_HW::DrawLine(const GPUBackendDrawLineCommand* cmd)
{
  ...
  GSVector4i start_pos = GSVector4i::loadl(&cmd->vertices[0].x);
  u32 start_color = cmd->vertices[0].color;

  for (u32 i = 1; i < num_vertices; i++)
  {
    const GSVector4i end_pos = GSVector4i::loadl(&cmd->vertices[i].x);
...
void GPU_HW::DrawSprite(const GPUBackendDrawRectangleCommand* cmd)
{
  ...
  for (u32 y_offset = 0; y_offset < rectangle_height;)
  {
    ...
    for (u32 x_offset = 0; x_offset < rectangle_width;)
    {
      ...
      if (cmd->rc.texture_enable && m_texpage_dirty != 0)
      {
...
void GPU_HW::DrawPolygon(const GPUBackendDrawPolygonCommand* cmd)
{
  ...
  for (u32 i = 0; i < num_vertices; i++)
  {
    ...
    vertices[i].Set(vert_pos.x, vert_pos.y, depth, 1.0f, vert.color, texpage, vert.texcoord, 0xFFFF0000u);
  }
                      ...
                      GPUBackendCommandParameters params)
{
  ...
  if (use_shader || IsUsingMultisampling())
  {
    ...
    g_gpu_device->SetPipeline(
      m_vram_copy_pipelines[BoolToUInt8(params.check_mask_before_draw && !m_pgxp_depth_buffer && NeedsDepthBuffer())]
        .get());
    g_gpu_device->PushUniformBuffer(&uniforms, sizeof(uniforms));
    g_gpu_device->Draw(3, 0);
    RestoreDeviceContext();
...
void GPU_HW::PrepareDraw(const GPUBackendDrawCommand* cmd)
{
  ...
  if (cmd->rc.IsTexturingEnabled())
  {
    ...
    if (((m_draw_mode.bits ^ cmd->draw_mode.bits) & GPUDrawModeReg::TEXTURE_PAGE_AND_MODE_MASK) != 0 ||
  ...
  if (texture_mode != m_batch.texture_mode || transparency_mode != m_batch.transparency_mode ||
      ...
      dithering_enable != m_batch.dithering || m_batch.set_mask_while_drawing != cmd->params.set_mask_while_drawing ||
...
void GPU_HW::DrawingAreaChanged(const GPUDrawingArea& new_drawing_area, const GSVector4i clamped_drawing_area)
{
  m_clamped_drawing_area = clamped_drawing_area;

...
static void FindTextures(bool load_vram_write_replacements, bool load_texture_replacements);
static void LoadTextureAliases(const ryml::ConstNodeRef& root, bool load_vram_write_replacement_aliases,
...
static constexpr const char LOCAL_CONFIG_TEMPLATE[] = R"(# DuckStation Texture Replacement Configuration
...
void TextureReplacements::FindTextures(bool load_vram_write_replacements, bool load_texture_replacements)
{
  FileSystem::FindResultsArray files;
  FileSystem::FindFiles(GetSourceDirectory().c_str(), "*", FILESYSTEM_FIND_FILES | FILESYSTEM_FIND_RECURSIVE, &files);
...
void TextureReplacements::LoadTextureAliases(const ryml::ConstNodeRef& root, bool load_vram_write_replacement_aliases,
                                             bool load_texture_replacement_aliases)
{
  const std::string source_dir = GetSourceDirectory();

  for (const ryml::ConstNodeRef& current : root.cchildren())
  {
    const std::string_view key = to_stringview(current.key());
    const std::optional<ReplacementType> type = GetReplacementTypeFromFileTitle(key);
    if (!type.has_value())
      continue;
...
bool TextureReplacements::EnsureGameDirectoryExists()
{
  ...
  if (const std::string config_path = Path::Combine(game_directory, LOCAL_CONFIG_FILENAME);
      ...
      !FileSystem::WriteStringToFile(config_path.c_str(), LOCAL_CONFIG_TEMPLATE, &error))
  {
...
bool TextureReplacements::LoadLocalConfiguration(bool load_vram_write_replacement_aliases,
                                                 bool load_texture_replacement_aliases)
{
  ...
  s_config.max_vram_write_splits = GetOptionalTFromObject<bool>(root, "MaxVRAMWriteSplits")
                                     .value_or(static_cast<bool>(s_config.max_vram_write_splits));
  s_config.texture_dump_width_threshold = GetOptionalTFromObject<bool>(root, "DumpTextureWidthThreshold")
                                            .value_or(static_cast<bool>(s_config.texture_dump_width_threshold));
  s_config.texture_dump_height_threshold = GetOptionalTFromObject<bool>(root, "DumpTextureHeightThreshold")
                                             .value_or(static_cast<bool>(s_config.texture_dump_height_threshold));
  s_config.vram_write_dump_width_threshold = GetOptionalTFromObject<bool>(root, "DumpVRAMWriteWidthThreshold")
                                               .value_or(static_cast<bool>(s_config.vram_write_dump_width_threshold));
  s_config.vram_write_dump_height_threshold = GetOptionalTFromObject<bool>(root, "DumpVRAMWriteHeightThreshold")
                                                .value_or(static_cast<bool>(s_config.vram_write_dump_height_threshold));
  ...
  if (load_vram_write_replacement_aliases || load_texture_replacement_aliases)
  {
    ...
    if (aliases.valid() && aliases.has_children())
      LoadTextureAliases(aliases, load_vram_write_replacement_aliases, load_texture_replacement_aliases);
  }

...
void GraphicsSettingsWidget::onTextureReplacementOptionsClicked()
{
  ...
  connect(dlgui.exportButton, &QPushButton::clicked, &dlg, [&dlg, &dlgui]() {
    ...
    const QString filename =
      QFileDialog::getSaveFileName(&dlg, tr("Save Configuration"), QString(), tr("Configuration Files (*.yaml)"));
    if (filename.isEmpty())
      return;

    INISettingsInterface sif(QDir::toNativeSeparators(filename).toStdString());
    if (FileSystem::FileExists(sif.GetFileName().c_str()) && !sif.Load())
    {
      QMessageBox::critical(&dlg, tr("Error"), tr("Failed to load existing ini."));
      return;
    }

    sif.SetBoolValue("Configuration", "DumpTexturePages", dlgui.dumpTexturePages->isChecked());
    sif.SetBoolValue("Configuration", "DumpC16Textures", dlgui.dumpC16Textures->isChecked());
    sif.SetBoolValue("Configuration", "ReducePaletteRange", dlgui.reducePaletteRange->isChecked());
    sif.SetBoolValue("Configuration", "ConvertCopiesToWrites", dlgui.convertCopiesToWrites->isChecked());
    sif.SetIntValue("Configuration", "MaxVRAMWriteSplits", dlgui.maxVRAMWriteSplits->value());
    sif.SetIntValue("Configuration", "DumpTextureWidthThreshold", dlgui.minDumpedTextureWidth->value());
    sif.SetIntValue("Configuration", "DumpTextureHeightThreshold", dlgui.minDumpedTextureHeight->value());
    sif.SetBoolValue("Configuration", "DumpTextureForceAlphaChannel", dlgui.setTextureDumpAlphaChannel->isChecked());

    sif.SetIntValue("Configuration", "DumpVRAMWriteWidthThreshold", dlgui.minDumpedVRAMWriteWidth->value());
    sif.SetIntValue("Configuration", "DumpVRAMWriteHeightThreshold", dlgui.minDumpedVRAMWriteHeight->value());
    sif.SetBoolValue("Configuration", "DumpVRAMWriteForceAlphaChannel", dlgui.setTextureDumpAlphaChannel->isChecked());

    Error error;
    if (!sif.Save(&error))
      QMessageBox::critical(&dlg, tr("Save Failed"), QString::fromStdString(error.GetDescription()));
#else
    Panic("fixme");
#endif
  });
